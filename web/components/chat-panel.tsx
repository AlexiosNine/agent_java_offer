  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      <ScrollArea className="flex-1 p-3 sm:p-4">
        {messages.length === 0 && (
          <div className="text-center text-muted-foreground mt-12 sm:mt-20 px-4">
            <p className="text-base sm:text-lg mb-2">开始提问吧</p>
            <p className="text-xs sm:text-sm">基于面试资料库的智能问答</p>
          </div>
        )}
        {messages.map((msg) => (
        ...
        {isLoading && (
          <div className="flex justify-start mb-4">
            <div className="bg-muted rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-muted-foreground">
              思考中...
            </div>
          </div>
        )}
